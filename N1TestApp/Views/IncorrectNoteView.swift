...
// 🌟 스와이프를 위한 Presentation Struct 추가
...
private struct ReadingNoteDetailView: View {
    ...
    @Environment(\.dismiss) private var dismiss // Container 닫기 용도
    @Environment(\.colorScheme) private var colorScheme
    private var cs: ColorScheme { colorScheme }

    // 🌟 인터랙티브 풀이를 위한 상태 변수 추가
                ...
                HStack(spacing: 16) {
                    ...
                    Button(action: { dismiss() }) {
                        Image(systemName: "xmark").font(.system(size: 14, weight: .bold))
                            .foregroundColor(cs == .dark ? .white.opacity(0.8) : .black.opacity(0.7)).padding(4)
                    }
                }
            }
        }
