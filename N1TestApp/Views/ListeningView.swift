...
struct ListeningView: View {
    ...
    private var audioPlayerPanel: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(spacing: 12) {
                ...
                // 🌟 정답 선택 후 나타나는 속도 조절 버튼 (타입 오류 수정 완료)
                if showAnswer {
                    Menu {
                        ...
                        .foregroundColor(storeManager.isPremium ? (cs == .dark ? .white.opacity(0.8) : Color.examBlue) : .orange)
    ...
    private func setupEndTimeTimer() {
        ...
        // 🌟 남은 시간을 재생 배속에 비례하여 계산 (배속이 빠르면 타이머 인터벌이 그만큼 짧아짐)
    ...
    private func changePlaybackRate(to rate: Float) {
        ...
        // 🌟 재생 중 속도를 낮추면 플레이어가 일시정지되는 현상을 방지하기 위해
        // play()를 다시 강제 호출해줍니다.
        if isPlaying {
            audioPlayer?.play()
            setupEndTimeTimer()
